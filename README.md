...
- **MCP**: `python -m hexset.server.mcp`, run alongside an already-running `web.py` (`HEXSET_UI_BASE_URL`, default `http://127.0.0.1:8770`). A thin stdio client of that same HTTP API, exposing `models`, `new_game`, `join`, `board`, `state`, `act`, `undo`, `leave_game`, `get_table`, `offer_trade`, `answer_trade`, `choose_trade` and `resume_game` as tools. `new_game`/`join` take a required `model` argument — your exact model identifier, e.g. `claude-opus-5` — hashed into the seat's `client` identity so `resume_game` can reclaim it later by that same string if its saved token stops working; there is no environment-variable override, the model supplies its own string. `act` takes an index into `state()`'s `legal_actions` and settles the whole bot cascade before returning, so one tool call is one full human turn, not one click. Every seat this connects is gated by `PendingGate` (see "Trading" below): nothing a bot's trade event or another seat's offer finds against it ever clears on its own. Hand-rolled against the MCP stdio wire format rather than built on the official SDK, which pulls in a compiled dependency (`pydantic`) this project otherwise has none of.
...
seat is claimed (the web page's own seat-up, or `hexset.server.mcp`'s
`new_game`/`join` — there is no flag to ask for anything else): a
