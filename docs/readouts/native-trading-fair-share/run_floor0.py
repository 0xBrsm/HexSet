...
def factory(entrant,board,rng):
    ...
    bot.trade_floor=TRADE_FLOOR
    assert bot.trade_floor==0.0
    return bot
...
def play(job):
    ...
    trace=hashlib.sha256();actions=0;calls=[0]*4;offers=[0]*4;above_floor=[0]*4;max_gain=[None]*4;finished=[]
    ...
    def gains(bot,view,received,counterparties):
        ...
        values=original_gains(bot,view,received,counterparties)
        seat=view.perspective;offers[seat]+=len(values)
        above_floor[seat]+=sum(v>bot.trade_floor for v in values)
        if values:
            peak=max(values);max_gain[seat]=peak if max_gain[seat] is None else max(max_gain[seat],peak)
        return values
    ...
    row=dict(identity=identity,complete=True,winner=outcome.winner,seating=outcome.seating,
             ...
             trade_gate_calls_by_seat=calls,offers_evaluated_by_seat=offers,
             offers_above_floor_by_seat=above_floor,max_trade_gain_by_seat=max_gain,
...
def main():
    ...
    identity=dict(protocol='native-trading-fair-share-v1',host='hexset',information_model='native-public-history',
                  ...
                  profiles=PROFILES,trade_floor=TRADE_FLOOR,trading='enabled-all-seats')
    ...
    for i in [0,N]:
        row=play(jobs[i])
        print(f"Trading gates verified: {row['identity']['opponent']}, {row['domestic_trades']} exchanges",flush=True)
    rows=[]
