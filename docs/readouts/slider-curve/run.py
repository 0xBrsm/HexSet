"""Frozen native direct-slider screen and fresh confirmation."""
import argparse
...
def make(profile,board,rng):
 if profile=='A':return heximax(board,rng,**PARAMS,adaptive=True)
 if profile=='balanced':
  from dataclasses import replace
  return arena.spawn(replace(arena.PRESETS['heximax-balanced'],temperature=WIN_TEMPERATURE),board,rng)
...
class ControlledBot:
 def __init__(self,bot,role,seed,profile):
  ...
  self.observer=bot.activity if profile=='A' else TradeActivity()
  self.events=[];self.trajectory=[]
 def choose(self,game):
  self.game=game
  alpha=self.observer.mean if self.profile=='A' else (1. if self.profile=='balanced' else float(self.profile)) if self.role<0 else None
  activity=self.observer.mean
 ...
 def observe_trade(self,**public):
  if self.role<0:
   if self.profile=='A':self.bot.observe_trade(**public)
   else:self.observer.observe(**public)
...
def initialize():arena.register_entrant_kind('slider-curve',factory)
...
def play(job):
 ...
 ent=lambda n:arena.Entrant(n,kind='slider-curve')
...
def identity_base():
 ...
 return dict(protocol='direct-slider-curve-v1',host='hexset',source_sha256=SOURCE,
             ...
             expansion=trading_profile(float(p))[1]) for p in FIXED},exchange_weights=asdict(TRADING_WEIGHTS),
...
def paired(groups,a,controls,z=1.959964):
 means=[];vs=[]
 ...
 return dict(difference=estimate,se=se,interval95=[estimate-z*se,estimate+z*se],lower975=estimate-z*se)
...
def selection(groups):
 def wins(p,regimes):return sum(r['winner']==0 for g in regimes for r in groups[g,p].values())
 def select(regimes):return min(FIXED,key=lambda p:(-wins(p,regimes),abs(float(p)-.5),float(p)))
 global_best=select(REGIMES);conditional={g:select([g]) for g in REGIMES}
 max_var=0
 for controls in ({g:global_best for g in REGIMES},conditional):
  vs=[]
  for g in REGIMES:
   d=[int(groups[g,'A'][i]['winner']==0)-int(groups[g,controls[g]][i]['winner']==0) for i in groups[g,'A']]
   vs.append(variance(d))
  max_var=max(max_var,mean(vs))
 target=(NormalDist().inv_cdf(.975)+NormalDist().inv_cdf(.8))**2*max_var/(len(REGIMES)*.02**2)
 n=min(1024,max(256,64*math.ceil(target/64)))
 return dict(global_best=global_best,conditional_best=conditional,confirmation_n=n,
             estimated_variance=max_var,uncapped_target_per_regime=target,power_cap_applied=target>1024)
...
def main():
 ...
 def job(regime,c,index,seed,purpose='screen'):
 ...
 if args.stage=='screen':
  for regime,a,b in [('none','A','0'),('full','1','balanced')]:
   for index in range(2):
    results=[]
    for c in (a,b):
     identity,path=job(regime,c,index,724900000,'preflight')
    ...
    for k in ('winner','seating','points','turns','actions','action_sha256','trades'):
     assert results[0][k]==results[1][k],(regime,index,k)
  print('Eight endpoint control games matched full actions and trade histories.',flush=True)
 jobs=[]
 for ri,regime in enumerate(REGIMES):
  if args.stage=='screen':candidates=FIXED+['A'];n=64;seed=724000000+ri*10000
  else:
   assert chosen is not None
   candidates=list(dict.fromkeys(['A',chosen['global_best'],chosen['conditional_best'][regime]]))
   n=chosen['confirmation_n'];seed=725000000+ri*10000
  for c in candidates:
   for index in range(n):jobs.append(job(regime,c,index,seed,args.stage))
 rows=[]
 ...
 if args.stage=='screen':result['selection']=selection(groups)
  ...
  result['primary']={name:paired(groups,'A',controls) for name,controls in [
   ('global',{g:chosen['global_best'] for g in REGIMES}),('conditional',chosen['conditional_best'])]}
  result['noninferiority_confirmed']=all(c['lower975']>-.02 for c in result['primary'].values())
