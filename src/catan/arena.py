...
def _spawn(entrant: Entrant, board: Board, rng: random.Random) -> Bot:
    ...
    if entrant.kind == "network":
        ...
        # onnxruntime: a "search2"-only table (or anything else that never
        # spawns a network/mcts entrant) shouldn't need it installed.
        from .onnxbot import network_bot
    ...
    if entrant.evaluator == "network":
        ...
        from .onnxbot import network_evaluator
...
def _play_one(
    ...
    seats_taken = [seat_of(e, index, seats) for e in range(seats)]

    lineup: list[Bot] = [None] * seats  # type: ignore[list-item]
    for e, entrant in enumerate(entrants):
        lineup[seats_taken[e]] = spawn(
            entrant, board, random.Random(f"{seed}:{index}:{e}")
        )

    game = play(
        lineup, board, random.Random(f"{seed}:{index}:game"), action_cap=action_cap
    )
...
def compete(
    ...
    *,
    ...
    jobs = [(lineup, i, seed, action_cap) for i in range(games)]
...
def entrant_from_name(name: str) -> Entrant:
    ...
    if name.startswith(NETWORK):
        return Entrant(name="network", kind="network", weights=name[len(NETWORK) :])
