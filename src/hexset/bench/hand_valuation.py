...
grid centre, which must read about 50% against itself). `gates` runs the
...
Everything here reads the vector through heximax's own default stance, which
is `win` -- a softmax over the per-seat scores at `search2.WIN_TEMPERATURE`.
That is not what the first version of this fit ran under. It ran under
`relative`, where the vector's global scale cancels out of every comparison
and so cannot be fitted at all; only the three terms' ratios to each other
were ever pinned. Under `win` the scale is identified, because scaling the
vector by k is exactly changing the temperature to T/k -- which is also why
`WIN_TEMPERATURE` has to be refitted after this sweep and not before: its
own fit was a maximum likelihood over score vectors produced by the terms
these cells are replacing.
"""
...
# `road_sweep.MAX_WORKERS` records an 8-worker ceiling and says in as many
# words that it is not to be raised because the load average looks low. That
# ceiling is about the box it was written on, which is shared. This fit runs
# on a 32-core machine the PI allocated to it outright on 2026-09-06;
# `road_sweep`'s own cap is left where it is, since it still runs elsewhere.
MAX_WORKERS = 30

# The grid, around the values the terms were written with. `buy_progress` is
# the scale on a hand that fully covers a purchase worth a victory point;
...
# be lost to a 7.
#
# All three are swept. The first fit swept two, on the argument that
# `spare_card` "adds the same quarter-card to every hand at the table and the
# search only ever reads differences" -- which is wrong, and measurably so.
# Hands differ in size and in how much of one a purchase commits, and each
# spare card is priced at that seat's own bank or port rate, so the term is
# not a constant offset. Read through the `win` stance over 3,048 one-for-one
# swaps, moving it 0.15 -> 0.05 shifts win probability by 0.0048 on average
# (max 0.043, changing which seat leads on 1.6% of positions), against 0.0031
# (max 0.011, 0.5%) for moving `buy_progress` 0.45 -> 0.30. The term left out
# of the first grid moved the vector further than the one swept in it.
...
CELLS: tuple[dict[str, float], ...] = (
    ...
    {**CENTRE, "spare_card": 0.05},
    {**CENTRE, "spare_card": 0.30},
