...
def affordable(purchase: Purchase, walk: Survey, deck_left: int) -> bool:
...
class Weights:
    ...
    These three values are NOT yet fitted for the bot that ships them. They
    are the term functions' original fit, which was made on a branch under
    the `relative` stance and never merged; `win` is the default now, and
    the two differ in a way that matters here. Under `relative` the global
    scale of this vector has no gradient at all -- scale every weight alike
