...
Usage, matching catanatron's own `--players` syntax:
    ...
    python -m hexset.catanatron.duel --players=DC:heximax,AB:2,AB:2,AB:2 \\
        --num=400 --workers=8
...
def build_players(players_spec: str) -> list:
    ...
    for part, color in zip(parts, Color):
        if part.split(":")[0].upper() == "DC":
            ...
            players.append(
                DevCatanPlayer(
                    color, DevCatanPlayer.Params(entrant=tail or "heximax")
                )
            )
        else:
...
def main() -> None:
    ...
    parser.add_argument("--players", required=True, help="catanatron --players syntax, e.g. DC:heximax,AB:2,AB:2,AB:2")
