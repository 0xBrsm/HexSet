...
"""Settings, provenance and raw arena outcomes for benchmark JSON output.
...
def result_document(
    ...
    *,
    ...
    _validate_result(tournament, entrants)
    settings = {
    ...
    # Convert dataclass settings and paths with the standard JSON encoder.
    return json.loads(json.dumps(document, default=_json_default, allow_nan=False))


def _validate_result(tournament: Tournament, entrants: Sequence[Entrant]) -> None:
    """Catch incorrect entrant mappings before attaching settings to a result."""
    n = len(entrants)
    if not 2 <= n <= 6 or tournament.games <= 0:
        raise ValueError("experiment requires 2–6 entrants and a positive game count")
    if [s.name for s in tournament.standings] != [e.name for e in entrants]:
        raise ValueError("standings must match the supplied entrant order")
    for field in ("points", "roads", "settlements", "cities"):
        rows = getattr(tournament, field)
        if any(len(row) != n or any(type(v) is not int or v < 0 for v in row) for row in rows):
            raise ValueError(f"{field} must contain one nonnegative integer per entrant")
    if any(type(t) is not int or t < 0 for t in tournament.turns):
        raise ValueError("turns must be nonnegative integers")
    for seating in tournament.seating:
        if any(type(s) is not int for s in seating) or sorted(seating) != list(range(n)):
            raise ValueError("seating must be a permutation of entrant seat indices")
    if any(w is not None and (type(w) is not int or not 0 <= w < n) for w in tournament.winners):
        raise ValueError("winner must be an entrant index or null")
    if tournament.unfinished != tournament.winners.count(None) or [s.wins for s in tournament.standings] != [tournament.winners.count(e) for e in range(n)]:
