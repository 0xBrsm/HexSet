...
def responders(state: GameState, offer: Offer) -> tuple[int, ...]:
    ...
    Order matters, because an offer stops at the first player to take it and so
    whoever is asked first gets first refusal. Absolute seat order hands that
    advantage permanently to the low seats. Going round from the proposer hands
    it to nobody: the proposer rotates, so over a game the advantage rotates
    with them. That is also how a table actually works.
