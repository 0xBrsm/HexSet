...
SRC = REPO_ROOT / "src"
PACKAGE_ROOTS = {"hexset": SRC / "hexset"}
...
def packaged_names(tmp_path_factory) -> tuple[str, ...]:
    ...
    shutil.copytree(SRC, clean / "src", ignore=NOT_IN_A_CLEAN_CHECKOUT)
    out = workspace / "wheel"
...
def test_the_wheel_carries_nothing_from_outside_the_package(packaged_names):
    """`packages.find` is scoped to `hexset`/`hexset.*`; tests/, models/ and
