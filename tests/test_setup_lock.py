...
from hexset_ui.actions import ActionType, apply, options_for
from hexset_ui.board.board import random_base_board
from hexset_ui.game import (
    Phase,
    _advance_setup,
    _in_second_setup_round,
    _next_unlocked,
    end_turn,
    lock_seat,
    roll_dice,
...
def test_next_unlocked_raises_only_when_every_seat_is_locked():
    ...
    assert _next_unlocked(game, 0) == 3
    game.locked = frozenset({1, 2, 3})
    assert _next_unlocked(game, 0) == 0  # wraps back to the only unlocked seat


def test_advance_setup_is_idempotent_once_resolved():
    game = start(_board(), 3, random.Random(1), first=0)
